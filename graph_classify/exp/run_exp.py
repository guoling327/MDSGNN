...
from mp.MDSGNN_graph import MDSGNN
...
def main(args):
    ...
    if args.model.startswith('gin') or args.model in ['MDSGNN']:  # load graph dataset
    ...
    #                  readout=args.readout,                    # readout
    #     ).to(device)
    elif args.model == 'MDSGNN':
        model = MDSGNN(args.max_petal_dim,
                         num_features,  # num_input_features
                         args.num_layers,  # num_layers
                         args.emb_dim,  # hidden
                         num_classes,  # num_classes
