...
def DataLoader(name, args):
    ...
    calculate_ = True if args.net in ['HiGCN', 'MDSGNN'] else False
   ...
   # device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    # HL_sparse_tensor = data.HL[2]  # 这里选择HL中的第一个稀疏张量进行SVD分解
    #
    # # 将稀疏张量转换为稠密张量
    #
    #
    # dense_tensor2 = data.HL[2].to_dense()
    #
    # # 使用torch.svd函数进行SVD分解
    # U2, S2, V2 = torch.svd(dense_tensor2)
    # print(U.shape)
    # print(S.shape)

    # edge_index = torch_geometric.utils.to_scipy_sparse_matrix(data.edge_index)  # (83,183)
    # # edge_index = normalize_adj(edge_index)
    # adj = sparse_mx_to_torch_sparse_tensor(edge_index)  # (83,183)
    # A = adj.to_dense()
    #
    # e, u = eigen_decompositon(A)
    # S = torch.FloatTensor(e)
    # U = torch.FloatTensor(u)
    # print("S_list0",S_list[0].shape)
    # print("S_list1",S_list[1].shape)
...
def DataLoader2(name, args):
    # calculate higher_order adj-matrix and  save
    calculate_ = True if args.net in ['HiGCN', 'MDSGNN','MDSGNN2'] else False
    #hl_path = osp.join('..\\data\\' + name + '\\HL_' + name + '.pt') # 存储hl的路径
    hl_path = osp.join('..', 'data', name + '\\HL_' + name + '.pt') # 存储hl的路径
...
def DataLoader3(name, args):
    ...
    if calculate_:
        ...
        print("Calucating higher-order laplacian matix...")
        print(hl_path)
        data.HL = creat_L_SparseTensor2(G, maxCliqueSize=args.Order)
        torch.save(data.HL, hl_path)
   # device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    device = torch.device('cuda:' + str(args.cuda) if torch.cuda.is_available() else 'cpu')
    S_list = []
