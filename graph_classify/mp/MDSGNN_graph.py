...
class MDSGNNLayer(nn.Module):

    def __init__(self, nbases, ncombines, prop_dropout=0.0):
        super(MDSGNNLayer, self).__init__()
...
class MDSGNN_prop(MessagePassing):
    def __init__(self, K, alpha, Order=2, bias=True, **kwargs):
        super(MDSGNN_prop, self).__init__(aggr='add', **kwargs)
...
class MDSGNN_Message(MessagePassing):
    ...
    def __init__(self, num_features, order, hidden, num_classess, dropout_rate, nn2, device,**kwargs):
       ...
       #self.classify = nn.Linear(nclass, nclass)
        ...
        self.layers = nn.ModuleList([MDSGNNLayer(2, hidden, self.dprate) for i in range(self.nlayer)])
...
class MDSGNN(torch.nn.Module):
    def __init__(self, max_petal_dim, num_features, num_layers, hidden, num_classes, device,readout='sum',
        ...
        self.conv1 = MDSGNN_Message(num_features, self.order, hidden, hidden, dropout_rate,
            Sequential(
                Linear(hidden, hidden),
                BN(hidden),
                conv_nonlinearity(),
        ...
        for i in range(num_layers - 1):
            self.convs.append(
                MDSGNN_Message(hidden, self.order, hidden, hidden, dropout_rate,
                    Sequential(
                        Linear(hidden, hidden),
                        BN(hidden),
                        conv_nonlinearity(),
